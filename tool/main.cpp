...
void print_ast(std::ostream& out, const cppast::cpp_file& file, bool as_json = false)
{
    // print file name
    ...
    cppast::visit(file, [&](const cppast::cpp_entity& e, cppast::visitor_info info) {
        ...
        else
        {
            ...
            {
                json json_object;
                to_json_x(json_object, e);
                ast_json.push_back(json_object);
                return true;
            }
...
try
{
    ...
    option_list.add_options("compilation")
        ...
        ("print_ast_as_json", "print the cpp ast nodes into json");
    // clang-format on
    ...
    else
    {
        ...
        if (!file)
            return 2;
        print_ast(std::cout, *file, options.count("print_ast_as_json"));
    }
}
